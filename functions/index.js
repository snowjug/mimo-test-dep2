  ...
  try {
    ...
    try {
      await db.runTransaction(async (transaction) => {
        ...
        const isLeaseActive = jobData.cleanupState === "claiming" &&
          jobData.cleanupLeaseExpiresAt &&
          (jobData.cleanupLeaseExpiresAt.toDate ? jobData.cleanupLeaseExpiresAt.toDate().getTime() : new Date(jobData.cleanupLeaseExpiresAt).getTime()) > currentMs;

        if (jobData.fileDeleted === true || isLeaseActive) {
          transactionFailedError = { status: 400, message: "Print code already redeemed or job not paid" };
          throw new Error("TX_ABORT");
        }
