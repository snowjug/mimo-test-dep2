...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const navItems = [
    ...
    { id: 'operations', label: 'Operations', icon: Printer, badgeText: 'Live', badgeType: 'live' },
    { id: 'kiosks', label: 'Kiosks', icon: HardDrive },
    { id: 'incidents', label: 'Incidents', icon: AlertTriangle, badgeCount: incidentCount, badgeType: 'alert' },
  ...
  return (
    <div className="h-full flex flex-col justify-between select-none font-sans bg-[#0A1728] text-[#F5F7FA]">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden">
        ...
        {!isCollapsed && (
          <div className="px-5 pt-5 pb-2">
            <span className="text-[11px] font-black text-[#6F89A3] uppercase tracking-wider">
              Navigation
            </span>
          </div>
        )}
      ...
      {/* Bottom User Area */}
      <div
        className={`p-3.5 border-t border-[#1D3A59] bg-[#07111F]/50 flex items-center ${
          isCollapsed ? 'justify-center' : 'justify-between'
        }`}
      >
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-2.5 min-w-0'}`}>
          <div
            title={isCollapsed ? 'Admin User (admin@mimo.in)' : undefined}
            className="w-9 h-9 rounded-xl bg-[#20D3A2]/15 text-[#20D3A2] flex items-center justify-center font-bold text-xs border border-[#20D3A2]/30 shrink-0"
          >
            <User size={16} />
          </div>
          {!isCollapsed && (
        ...
        {!isCollapsed && onLogout && (
          <button
            ...
            onClick={onLogout}
            title="Logout"
            className="p-2 rounded-lg text-[#8EA6BF] hover:text-rose-400 hover:bg-rose-500/10 transition-all cursor-pointer shrink-0"
          >
            <LogOut size={16} />
          </button>
        )}
